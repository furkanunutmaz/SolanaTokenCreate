...
async function main() {
  ...
  const mint = await createNewMint(
    connection,
    user,           //  PAYER            We'll pay the fees
    user.publicKey, //  MINTAUTHORITY    We're the mint authority
    user.publicKey, //  FREEZEAUTHORITY  And the freeze authority >:)
    2               //  DECIMAL          Only two decimals!
  )
